 ...
 * Updated by YUDOUFU123 on 2020/5/17
 */
public class HDFSUtil
{
    ...
    public synchronized static Map<String, String> getFileSystemInformation(FileStatus fileStatus)
    {
        ...
        } else
        {
            ...
            fileSystemInformations.put(resourceBundle.getString(LanguageKeyWord.FILEREPLICATION), Short.toString(fileStatus.getReplication()));
            ...
            throws IOException
    {
        if (srcFS == dstFS)
        {
            //Update by YUDOUFU123 on 2020/5/17
            String srcq = src.makeQualified(srcFS.getUri(), srcFS.getWorkingDirectory()).toString() + Path.SEPARATOR;
            String dstq = dst.makeQualified(dstFS.getUri(), dstFS.getWorkingDirectory()).toString() + Path.SEPARATOR;
